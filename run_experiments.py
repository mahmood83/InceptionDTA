...
from keras.layers import Input, Embedding, LSTM, Dense, TimeDistributed, Masking, RepeatVector, Flatten, Add
from keras.models import Model
...
def inceptionDTA(FLAGS, NUM_FILTERS, FILTER_LENGTH1, FILTER_LENGTH2):
    ...
    plot_model(interactionModel, to_file='figures/inceptionDTA.png')
...
def general_nfold_cv(XD, XT,  Y, label_row_inds, label_col_inds, prfmeasure, runmethod, FLAGS, labeled_sets, val_sets): ## BURAYA DA FLAGS LAZIM????
    ...
    for foldind in range(len(val_sets)):
        ...
        pointer = 0
        train_generator = DataGenerator(train_drugs, train_prots,  train_Y,  pro2vec, batchsz, max_prot_len, index2char, shuffle= False)       
        validation_generator = DataGenerator(val_drugs, val_prots,  val_Y,  pro2vec, batchsz, max_prot_len, index2char, shuffle= False)       
        for param1ind in range(len(paramset1)): #hidden neurons
            ...
            for param2ind in range(len(paramset2)): #learning rate
                ...
                for param3ind in range(len(paramset3)):
                    ...
                    es = EarlyStopping(monitor='val_cindex_score', mode='max', verbose=1, patience=15,restore_best_weights=True,)
                    mc = ModelCheckpoint('best_modele_kiba.h5', monitor='val_cindex_score', mode='max', verbose=1, save_best_only=True)
                    gridres = gridmodel.fit_generator(generator=train_generator,validation_data=validation_generator, epochs=epoch, callbacks=[es, mc] ) 
